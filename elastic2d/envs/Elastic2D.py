...
def rand():
    return (random.random() / 2 + 0.5) * 3000 * random.choice([-1, 1])
...
class Elastic2D(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        def my_draw_circle(circle, body, fixture):
            ...
            pygame.draw.circle(self.screen, body.userData["color"], [int(x) for x in position],
    ...
    def reset(self):
        while True:
            ...
            for idx in range(self.numofobjs):
                ...
                if obj == "square":
                    temp_fixture = temp_body.CreatePolygonFixture(box=(size, size), density=1, friction=0.3,
                                                                  restitution=1.0)
                elif obj == "triangle":
                    temp_fixture = temp_body.CreatePolygonFixture(
                        shape=polygonShape(vertices=[(x * size, y * size) for (x, y) in TRIANGLE_POLY]),
                        density=1, friction=0.3, restitution=1.0)
                elif obj == "iron":
                    temp_fixture = temp_body.CreatePolygonFixture(
                        shape=polygonShape(vertices=[(x * size, y * size) for (x, y) in IRON_POLY]),
                        density=1, friction=0.3, restitution=1.0)
                elif obj == "hexagon":
                    temp_fixture = temp_body.CreatePolygonFixture(
                        shape=polygonShape(vertices=[(x * size, y * size) for (x, y) in HEXAGON_POLY]),
                        density=1, friction=0.3, restitution=1.0)
                else:  # circle
            ...
            for obj in self.world.bodies:
                if not (obj.userData["shape"] == "wall"):
                    if not (1 < obj.position[0] < ((SCREEN_WIDTH // PPM) - 1) and 1 < obj.position[1] < (
                            (SCREEN_HEIGHT // PPM) - 1)):
    ...
    def step(self, action):
        ...
        # self.objects[0].ApplyForceToCenter([discrete_action_x[int(action)] * 100, discrete_action_y[int(action)] * 100],
        #                                    True)
